...
2.  **Команда (Command)**:
    - **Реализация**: Классы в директории `commands`, реализующие интерфейс `ICommand` (например, `CreateBankAccountCommand`, `EditBankAccountCommand`, `DeleteBankAccountCommand`). `CommandProcessor` выступает в роли инвокера, обрабатывая команды и применяя к ним декоратор для замера времени выполнения.
